/*1481474405,,JIT Construction: v2732669,en_US*/
...
}  }).call(global);})(window.inDapIF ? parent.window : window, window);} catch (e) {new Image().src="https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"2732669","namespace":"FB","message":"'+e.message+'"}}');}

FB.init({
    appId      : '223576144703517',                        // App ID from the app dashboard
    status     : true,                                 // Check Facebook Login status
    xfbml      : true                                  // Look for social plugins on the page
});
function callbackFn(evt) {
    if(evt.status === "connected") {
        $( "#fb-root" ).trigger( "click");
        //$( "#fb-root" ).trigger( "fbLoggedIn", [ "loggedIn", "Event" ] );
    }
}
